...
from ascdro.algorithms import ASCDROConfig, ASCDROTrainer, DPSpiderConfig
from ascdro.datasets import CIFAR10STNPZ, build_transforms
from ascdro.models import build_resnet_cifar
from ascdro.risk import RiskModel
from ascdro.utils import AverageMeter, count_parameters, get_device, set_seed
...
def prepare_dataloaders(
    ...
    # image_size retained for API compatibility; CIFAR10-ST is fixed at 32x32.
    del image_size
    data_root = data_root.expanduser()
    train_path = data_root / "train.npz"
    test_path = data_root / "test.npz"
    train_dataset = CIFAR10STNPZ(train_path, transform=build_transforms(train=True))
    test_dataset = CIFAR10STNPZ(test_path, transform=build_transforms(train=False))

    train_dataset = _apply_debug_subset(train_dataset, debug_samples)
    test_dataset = _apply_debug_subset(test_dataset, debug_samples)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train ResNet20 on CIFAR10-ST using ASCDRO")
    parser.add_argument("--data-root", type=Path, required=True, help="Directory containing train.npz/test.npz for CIFAR10-ST")
    ...
    parser.add_argument("--epochs", type=int, default=30, help="Target epoch count for scheduling")
    parser.add_argument(
        "--loss-scale",
        type=float,
        default=None,
    ...
    parser.add_argument("--model-depth", type=int, default=20, help="ResNet depth (20 corresponds to standard CIFAR ResNet)")
    parser.add_argument("--image-size", type=int, default=32, help="Placeholder for CIFAR image size (unused)")
...
def main() -> int:
    ...
    def model_builder() -> torch.nn.Module:
        return build_resnet_cifar(depth=args.model_depth, num_classes=num_classes)
    ...
    if args.epochs is not None:
        total_steps = min(total_steps, int(args.epochs * steps_per_epoch))
    ...
    final_acc = val_acc_history[-1] if val_acc_history else 0.0
    best_acc = max(val_acc_history) if val_acc_history else final_acc
    best_step = next(
        (
            entry.get("step")
            for entry in history
            if entry.get("val_acc") is not None and float(entry["val_acc"]) / 100.0 == best_acc
        ),
        None,
    )
    avg_acc = sum(val_acc_history) / len(val_acc_history) if val_acc_history else final_acc
    ...
    print(
        f"[dro2] final_acc={summary['final_accuracy']:.4f}, "
